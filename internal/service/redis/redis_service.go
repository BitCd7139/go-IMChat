...
func GetKeyNilError(key string) (string, error) {
	value, err := redisClient.Get(ctx, key).Result()
	if err != nil {
		return "", err
	}
...
func GetKeyWithPrefixNilError(prefix string) (string, error) {
	var keys []string
	var err error
	for {
		...
		if len(keys) == 0 {
			zlog.Info("keys is empty")
			return "", redis.Nil
		}

		if len(keys) == 1 {
			zlog.Info(fmt.Sprintln("find key: ", keys))
		} else {
			zlog.Info(fmt.Sprintln("find multi keys: ", keys))
			return "", errors.New(fmt.Sprintln("find multi keys: ", keys))
		}
	}
}
...
func IncrRedisKeys(keys string) (value int64, err error) {
	value, err = redisClient.Incr(ctx, keys).Result()
	if err != nil {
		return -1, err
	}
	return value, nil
...
func IncrRedisWithDefault(key string, defaultValue int64) (value int64, err error) {
	value, err = redisClient.Incr(ctx, key).Result()
	if err != nil {
		if errors.Is(err, redis.Nil) {
			redisClient.Set(ctx, key, defaultValue, 0)
...
func SetKeyWithSets(key string, value string, timeout time.Duration) error {
...
func FindKeyWithSets(key string) ([]string, error) {
	values, err := redisClient.SMembers(ctx, key).Result()
	if err != nil {
		if errors.Is(err, redis.Nil) {
			return []string{}, nil
		}
